...
public class ButtonInteractions : MonoBehaviour
{
    ...
    {
        float disabled_scale = 0.7f;

        for(int i = 1; i < 21; i++)
        {
            _btn.localScale = Vector3.Lerp(_btn.localScale, Vector3.one * disabled_scale, i / 20f);
    ...
    IEnumerator Enabling(Transform _btn)
    {
        ...
        for (int i = 1; i < 21; i++)
        {
            _btn.localScale = Vector3.Lerp(_btn.localScale, Vector3.one * enabled_scale, i / 20f);
