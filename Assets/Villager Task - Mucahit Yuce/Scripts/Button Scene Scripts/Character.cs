...
public class Character : MonoBehaviour
{
    ...
    bool run, walkWithItem, walkWithHammer;
    ...
    void Run()
    {
        transform.position += (currentTarget.position - transform.position).normalized * runSpeed * Time.deltaTime;
    ...
    void WalkWithItem()
    {
        transform.position += (collectionPos - transform.position).normalized * walkWithItemSpeed * Time.deltaTime;

        if (Vector3.Distance(transform.position, collectionPos) <= 1.2f)
        {
            DropItem();
            walkWithItem = false;
        }
    ...
    void WalkWithHammer()
    {
        transform.position += (currentTarget.position - transform.position).normalized * walkWithHammerSpeed * Time.deltaTime;
